        ...
        <div class="col-md-6 mb-3" v-for="item in users.data" :key="item">
          <div class="card">
            <div class="card-body">
              ...
              <ul class="list-group" v-if="!['crud_admin', 'admin'].includes(current_user.role)">
                ...
                <Collapse :id="'pay-' + item.id">
                  <form @submit.prevent="payUser(item)">
                    <div class="input-group" :currency="$util.currency(item.payment.money)">
                      <input class="form-control" type="number" min="0" step="any" placeholder="Summa:"
                        v-model="item.payment.money" required />
                      <div class="input-group-append">
                        <DataDropdown type="currency" property="currency" v-model="item.payment.currency"></DataDropdown>
                      </div>
                    </div>
                    <div class="input-group">
                      <input class="form-control" type="text" placeholder="Izoh:" required
                        v-model="item.payment.comment" />
                    </div>
                    <button type="button" class="btn btn-sm btn-danger mt-2 mx-1 float-end" @click="
                      item.payment = {
                        currency: null,
                        money: 0,
                        currency_id: 0,
                      ...
                      ">
                      <img src="../../assets/icons/Cancel-white.svg" alt="Cancel" />
                    </button>
                    <button class="btn btn-sm btn-success mt-2 mx-1 float-end" :disabled="!item.payment.currency">
                      <img src="../../assets/icons/Done_round-white.svg" alt="Done_round" />
                    </button>
                  </form>
                </Collapse>
              </ul>
              <div class="row mt-3">
                <div class="col" v-if="!['crud_admin', 'admin'].includes(current_user.role)">
                  <RouterLink class="btn btn-sm w-100 btn-primary" :to="{
                    path: '/user',
                ...
                <div class="col" v-if="current_user.role !== 'crud_admin'">
                  <RouterLink class="btn btn-sm w-100 btn-warning" :to="{
                    path: '/user',
